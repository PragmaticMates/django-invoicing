...
class InvoiceMrpExporterMixin(ExporterMixin):
    ...
    def write_data(self, output):
        ...
        for invoice in self.get_queryset():
            ...
            etree.SubElement(invoice_elem, "TaxCode").text = str(getattr(invoice, 'vat_type', 0) or 41)
            ...
            etree.SubElement(invoice_elem, "DoubleEntryBookkeepingCode").text = "3" if invoice.origin == invoice.ORIGIN.INCOMING else "11"
            ...
            etree.SubElement(invoice_elem, "VariableSymbol").text = str(invoice.variable_symbol or '')
            etree.SubElement(invoice_elem, "ConstantSymbol").text = str(invoice.constant_symbol or '')
            etree.SubElement(invoice_elem, "SpecificSymbol").text = str(invoice.specific_symbol or '')
            etree.SubElement(invoice_elem, "OriginalDocumentNumber").text = invoice.related_document or ""
            ...
            if hasattr(invoice, "orders") and invoice.orders.exists():
                ...
                etree.SubElement(invoice_elem, "OrderNumber").text = first_order.order_number

            etree.SubElement(invoice_elem, "UserField1").text = str(invoice.reference or "")
            ...
            if invoice.bank_iban or invoice.bank_name:
                ...
                etree.SubElement(bank, "Name").text = invoice.bank_name or ""
                etree.SubElement(bank, "IBAN").text = invoice.bank_iban or ""
                etree.SubElement(bank, "BIC").text = invoice.bank_swift_bic or ""
        ...
        # Validate XML against XSD schema before writing
...
class OutgoingInvoiceMrpExporter(InvoiceMrpExporterMixin):
    ...
    def fill_company(self, invoice, parent_element):
        etree.SubElement(parent_element, "CompanyId").text = invoice.customer_registration_id or ""
        etree.SubElement(parent_element, "Name").text = invoice.customer_name or ""
        etree.SubElement(parent_element, "Street").text = invoice.customer_street or ""
        etree.SubElement(parent_element, "ZipCode").text = invoice.customer_zip or ""
        etree.SubElement(parent_element, "City").text = invoice.customer_city or ""
        etree.SubElement(parent_element, "CountryCode").text = getattr(invoice.customer_country, "code", "") or ""
        etree.SubElement(parent_element, "VatNumber").text = invoice.customer_vat_id or ""
        etree.SubElement(parent_element, "Phone").text = invoice.customer_phone or ""
        etree.SubElement(parent_element, "Email").text = invoice.customer_email or ""
...
class IncomingInvoiceMrpExporter(InvoiceMrpExporterMixin):
    ...
    def fill_company(self, invoice, parent_element):
        etree.SubElement(parent_element, "CompanyId").text = invoice.supplier_registration_id or ""
        etree.SubElement(parent_element, "Name").text = invoice.supplier_name or ""
        etree.SubElement(parent_element, "Street").text = invoice.supplier_street or ""
        etree.SubElement(parent_element, "ZipCode").text = invoice.supplier_zip or ""
        etree.SubElement(parent_element, "City").text = invoice.supplier_city or ""
        etree.SubElement(parent_element, "CountryCode").text = getattr(invoice.supplier_country, "code", "") or ""
        etree.SubElement(parent_element, "VatNumber").text = invoice.supplier_vat_id or ""
        etree.SubElement(parent_element, "Phone").text = invoice.issuer_phone or ""
        etree.SubElement(parent_element, "Email").text = invoice.issuer_email or ""
